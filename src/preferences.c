...
char prefFileLocation[] =
    "../assets/userPreferences.txt"; // preference file absolute path
FILE *fUserPref;
FILE *fUserTemp;
...
const char *prefElementNames[TOTAL_ELEMENTS] = {
    "Create New Profile",         "Change Username",
    "Change License Plate",       "Change Handicap Status",
    "Change EV Status",           "Change Location Preference",
    "Change Isolation Preference"};

const char *handicappedElementNames[HANDIELEMENTS] = {"Not Handicapped",
                                                      "Handicapped"};

const char *EVElementNames[EVELEMENTS] = {"Not EV", "EV"};

char *
strupr(char *s) { // simple function to turn lowercase into uppercase chars
  if (!s)
    return NULL;
  char *p = s;
  while (*p) {
...
void getPreferences(
  ...
  if (userProfile == 0) { // when called with argument 0 should read file and
                          // count amount of profiles
    printf(
    ...
    numberOfProfiles = 0;
    while (fscanf(fUserTemp, "%s , %s , %d , %d , %d , %d", tempUser.username,
                  tempUser.licensePlate, &tempUser.is_handicapped,
                  &tempUser.is_ev, &tempUser.prefClose,
                  &tempUser.prefIsolated) == 6) {
      numberOfProfiles++;
      printf("Profile:  %d, Username: %s, License Plate: %s, Handicapped "
             "and/or EV: %d, %d, Parking Preference: %d, %d\n",
             numberOfProfiles, tempUser.username, tempUser.licensePlate,
             tempUser.is_handicapped, tempUser.is_ev, tempUser.prefClose,
             tempUser.prefIsolated);
    }
    printf("A total of %d profiles were loaded successfully\n",
           numberOfProfiles);
    fclose(fUserTemp);

    if (strlen(currentUser.username) <
        1) { // if no profile is set as current, reads first (default) profile
             // to current
      getPreferences(1);
      currentUserNR = 1;
    }
    ...
    // ✅ FIX: Only close if the file is already open
    if (fUserPref != NULL) {
      fclose(fUserPref);
    }
    ...
    for (int i = 1; i <= userProfile; i++) {
      if (i < userProfile) {
        if (fscanf(fUserPref, "%s , %s , %d , %d , %d , %d", tempUser.username,
                   tempUser.licensePlate, &tempUser.is_handicapped,
                   &tempUser.is_ev, &tempUser.prefClose,
                   &tempUser.prefIsolated) == 6) {
        ...
        if (fscanf(fUserPref, "%s , %s , %d , %d , %d , %d",
                   currentUser.username, currentUser.licensePlate,
                   &currentUser.is_handicapped, &currentUser.is_ev,
                   &currentUser.prefClose, &currentUser.prefIsolated) == 6) {
    ...
    fclose(fUserPref);
    fUserPref = NULL; // ✅ Set to NULL after closing
  }
}
void savePreferences(
    typeBoolean makeDefault) { // function to save preference profiles to file
  if (makeDefault == 0) { // when called with argument 0 it adds the profile at
                          // the end of the file
    fclose(fUserPref);
    fUserPref = fopen(prefFileLocation, "a");
    fprintf(fUserPref, "\n%s , %s , %d , %d , %s , %s", tempName,
            strupr(tempPlate), tempHandi, tempEV, tempClose, tempIso);
    fclose(fUserPref);

  } else if (makeDefault == 1) { // when called with argument 1 it adds the
                                 // profile at the top of the file
    fclose(fUserPref);

    struct userPref *allUsers =
        calloc(numberOfProfiles + 1, sizeof(struct userPref));
    ...
    for (int i = 1; i <= numberOfProfiles; i++) {
      if (fscanf(fUserPref, "%s , %s , %d , %d , %d , %d", allUsers[i].username,
                 strupr(allUsers[i].licensePlate), &allUsers[i].is_handicapped,
                 &allUsers[i].is_ev, &allUsers[i].prefClose,
                 &allUsers[i].prefIsolated) == 6) {
    ...
    for (int j = 0; j < numberOfProfiles + 1; j++) {
      printf("Profile %d: %s, %s, %d, %d, %d, %d", j + 1, allUsers[j].username,
             strupr(allUsers[j].licensePlate), allUsers[j].is_handicapped,
             allUsers[j].is_ev, allUsers[j].prefClose,
             allUsers[j].prefIsolated);
      if (j == 0) {
        fprintf(fUserPref, "%s , %s , %d , %d , %d , %d", allUsers[j].username,
                strupr(allUsers[j].licensePlate), allUsers[j].is_handicapped,
                allUsers[j].is_ev, allUsers[j].prefClose,
                allUsers[j].prefIsolated);
      } else {
        fprintf(fUserPref, "\n%s , %s , %d , %d , %d , %d",
                allUsers[j].username, strupr(allUsers[j].licensePlate),
                allUsers[j].is_handicapped, allUsers[j].is_ev,
                allUsers[j].prefClose, allUsers[j].prefIsolated);
      }
    }
    fclose(fUserPref);
    free(allUsers);
...
void setPreferences() { // function to alter data in a specific profile
  ...
  for (int i = 0; i < numberOfProfiles; i++) {
    ...
    if (i + 1 == currentUserNR) {
      printf("\ncurrent: %s, allUsers: %s i: %d", currentUser.username,
             allUsers[i].username, i);
      printf("\ncurrent: %s, allUsers: %s\n", currentUser.licensePlate,
             allUsers[i].licensePlate);
      for (int h = 0; h <= strlen(tempName); h++) {
        if (h == strlen(tempName)) {
  ...
  for (int k = 0; k < numberOfProfiles; k++) {
    printf("%s , %s , %d , %d , %d , %d\n", allUsers[k].username,
           strupr(allUsers[k].licensePlate), allUsers[k].is_handicapped,
           allUsers[k].is_ev, allUsers[k].prefClose, allUsers[k].prefIsolated);
    if (k == 0) {
      fprintf(fUserPref, "%s , %s , %d , %d , %d , %d", allUsers[k].username,
              strupr(allUsers[k].licensePlate), allUsers[k].is_handicapped,
              allUsers[k].is_ev, allUsers[k].prefClose,
              allUsers[k].prefIsolated);
    } else {
      fprintf(fUserPref, "\n%s , %s , %d , %d , %d , %d", allUsers[k].username,
              strupr(allUsers[k].licensePlate), allUsers[k].is_handicapped,
              allUsers[k].is_ev, allUsers[k].prefClose,
              allUsers[k].prefIsolated);
    }
...
void changePreferences() { // function which draws the constant menu elements
                           // for preference GUI
  DrawText(TextFormat("User Preferences" /*" Current Option %d"*/, prefOption),
           280, 80, 24, BLACK);
  changePrefGui();
...
int changePrefGui() {   // function which draws the specific menu elements for
                        // preference GUI
  ...
  default:              // main preference menu
    ...
    int posX1 = GetScreenWidth() / 2 -
                (elementGap + elementWidth) * (TOTAL_ELEMENTS - 1) / 4;
    int posX2 = GetScreenWidth() / 2 -
                (elementGap + elementWidth) * (TOTAL_ELEMENTS - 1) / 4;
    if (GuiButton((Rectangle){20, 70, 150, 20}, prefElementNames[0])) {
      prefOption = (changeType)0;
    }
    if (GuiButton((Rectangle){(GetScreenWidth() - 250), 70, 150, 20},
    ...
    for (int n = 1; n < TOTAL_ELEMENTS; n++) {
      if (n <= 3) {
        if (GuiButton((Rectangle){posX1, 130, elementWidth, 20},
                      prefElementNames[n])) {
          prefOption = (changeType)n;
        }
        posX1 += elementWidth + elementGap;
      } else if (n > 3) {
        if (GuiButton((Rectangle){posX2, 155, elementWidth, 20},
                      prefElementNames[n])) {
          prefOption = (changeType)n;
        }
  ...
  case 0: // submenu for creating a new profile
    ...
    if (GuiTextBox((Rectangle){boxX2, 120, 400, 30}, tempName, 21,
                   textBoxEditable[0])) {
      textBoxEditable[0] = !textBoxEditable[0];
    }
    if (GuiTextBox((Rectangle){boxX2, 160, 400, 30}, tempPlate, 8,
                   textBoxEditable[1])) {
      textBoxEditable[1] = !textBoxEditable[1];
    }
    if (GuiButton((Rectangle){boxX2, 200, 400, 30},
                  handicappedElementNames[tempHandi])) {
      tempHandi = !tempHandi;
    }
    if (GuiButton((Rectangle){boxX2, 240, 400, 30}, EVElementNames[tempEV])) {
      tempEV = !tempEV;
    }
    if (GuiTextBox((Rectangle){boxX2, 280, 400, 30}, tempClose, 2,
                   textBoxEditable[2])) {
      textBoxEditable[2] = !textBoxEditable[2];
    }
    if (GuiTextBox((Rectangle){boxX2, 320, 400, 30}, tempIso, 2,
                   textBoxEditable[3])) {
      textBoxEditable[3] = !textBoxEditable[3];
    }
    if (GuiButton((Rectangle){GetScreenWidth() / 2 - 120, 400, 90, 20},
                  "Make Default")) {
      savePreferences(1);
      tempName[0] = '\0';
      tempPlate[0] = '\0';
    ...
    if (GuiButton((Rectangle){GetScreenWidth() / 2 - 25, 400, 50, 20},
                  "Confirm")) {
      savePreferences(0);
      tempName[0] = '\0';
      tempPlate[0] = '\0';
  ...
  case 1: // submenu for changing username
    DrawText(TextFormat("Current Profile: %s", currentUser.username), 290, 100,
             20, BLACK);
    DrawText("Input New Name", 300, 200, 20, BLACK);
    if (GuiTextBox((Rectangle){GetScreenWidth() / 2 - 200, 220, 400, 30},
                   tempName, 21, textBoxEditable[0])) {
      textBoxEditable[0] = !textBoxEditable[0];
    }
    if (GuiButton((Rectangle){GetScreenWidth() / 2 + 205, 220, 50, 30},
                  "Change")) {
  ...
  case 2: // submenu for changing license plate
    DrawText(TextFormat("Current Profile and License Plate: %s, %s",
                        currentUser.username, currentUser.licensePlate),
             200, 100, 20, BLACK);
    DrawText("Input New License Plate", 300, 200, 20, BLACK);
    if (GuiTextBox((Rectangle){GetScreenWidth() / 2 - 200, 220, 400, 30},
                   tempPlate, 8, textBoxEditable[0])) {
      textBoxEditable[0] = !textBoxEditable[0];
    }
    if (GuiButton((Rectangle){GetScreenWidth() / 2 + 205, 220, 50, 30},
                  "Change")) {
  ...
  case 3: // submenu for changing handicapped status
    DrawText(TextFormat("Current Profile: %s", currentUser.username), 290, 100,
             20, BLACK);
    if (GuiButton((Rectangle){GetScreenWidth() / 2 - 200, 220, 400, 30},
                  handicappedElementNames[currentUser.is_handicapped])) {
  ...
  case 4: // submenu for changing EV status
    DrawText(TextFormat("Current Profile: %s", currentUser.username), 290, 100,
             20, BLACK);
    if (GuiButton((Rectangle){GetScreenWidth() / 2 - 200, 220, 400, 30},
                  EVElementNames[currentUser.is_ev])) {
  ...
  case 5: // submenu for changing proximity preference
    DrawText(TextFormat("Current Profile and Proximity Preference: %s, %d",
                        currentUser.username, currentUser.prefClose),
             200, 100, 20, BLACK);
    DrawText("Input New Proximity Preference", 300, 200, 20, BLACK);
    if (GuiTextBox((Rectangle){GetScreenWidth() / 2 - 200, 220, 400, 30},
                   tempClose, 2, textBoxEditable[0])) {
      textBoxEditable[0] = !textBoxEditable[0];
    }
    if (GuiButton((Rectangle){GetScreenWidth() / 2 + 205, 220, 50, 30},
                  "Change")) {
  ...
  case 6: // submenu for changing isolation preference
    DrawText(TextFormat("Current Profile and Isolation Preference: %s, %d",
                        currentUser.username, currentUser.prefIsolated),
             200, 100, 20, BLACK);
    DrawText("Input New Isolation Preference", 300, 200, 20, BLACK);
    if (GuiTextBox((Rectangle){GetScreenWidth() / 2 - 200, 220, 400, 30},
                   tempIso, 2, textBoxEditable[0])) {
      textBoxEditable[0] = !textBoxEditable[0];
    }
    if (GuiButton((Rectangle){GetScreenWidth() / 2 + 205, 220, 50, 30},
                  "Change")) {
  ...
  case 7: // submenu for changing active profile
    ...
    if (GuiButton((Rectangle){GetScreenWidth() / 2 + 205, 220, 50, 30},
      ...
      if (strtod(tempName, &intEnd) != currentUserNR) {
        getPreferences(0);
        if (strtod(tempName, &intEnd) <= numberOfProfiles &&
            tempName[0] != '\0' && strtod(tempName, &intEnd) != 0) {

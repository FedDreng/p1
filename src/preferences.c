...
void getPreferences(
  ...
  while (fscanf(fUserTemp, "%s %s %d %d %d %d", tempUser.username,
    ...
    numberOfProfiles++;
    printf("Profile: %d, Username: %s, License Plate: %s, Handicapped and/or "
           "EV: %d, %d, Parking Preference: %d, %d\n",
  ...
  if (userProfile == 0) { // when called with argument 0 should read all
                          // profiles and create struct to house them
    ...
    for (int i = 1; i <= userProfile; i++) {
      if (i < userProfile) {
        ...
        if (fscanf(fUserPref, "%s %s %d %d %d %d", currentUser.username,
                   currentUser.licensePlate, &currentUser.is_handicapped,
                   &currentUser.is_ev, &currentUser.prefClose,
                   &currentUser.prefIsolated) == 6) {
...
void savePreferences(
    boolean makeDefault) { // function to save preference profiles to file
  if (makeDefault == 0) {
    fclose(fUserPref);
    fUserPref = fopen(prefFileLocation, "a");
    fprintf(fUserPref, "\n%s %s %d %d %s %s", tempName, strupr(tempPlate),
            tempHandi, tempEV, tempClose, tempIso);
    fclose(fUserPref);
  }
}
...
int changePrefGui() {
  ...
  case 0:
    ...
    if (GuiButton((Rectangle){GetScreenWidth() / 2 - 25, 390, 50, 20},
                  "Confirm")) {
      savePreferences(0);
      tempName[0] = '\0';
      tempPlate[0] = '\0';
